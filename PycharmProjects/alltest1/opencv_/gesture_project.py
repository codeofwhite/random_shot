    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                x = int(hand_landmarks.landmark[0].x * img.shape[1])  # 取得食指末端 x 座標
                y = int(hand_landmarks.landmark[0].y * img.shape[0])  # 取得食指末端 y 座標
                ...
                if x > rx and x < (rx + 80) and y > ry and y < (ry + 80):
                    run = True
                # 將節點和骨架繪製到影像中
                cv2.putText(img, f"{label}: {score:.2f}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255),
                            2)
